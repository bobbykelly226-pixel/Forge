...
--
-- Corrected in place prior to first apply (section 10):
-- revoke direct table writes, operation_id idempotency, explicit empty answers
-- for min_selections = 0, duplicate key rejection, typed JSON validation,
-- resume-aware in_progress status, and grant/policy verification.
-- Do not rewrite this file if it is already recorded in linked migration history;
-- add a forward-only hardening migration instead.
...
-- ---------------------------------------------------------------------------
-- 10. Persistence hardening: revoke direct writes, idempotency, empty answers
-- ---------------------------------------------------------------------------

-- A. Revoke direct table mutation access (RPCs are the sole write boundary)
revoke insert, update, delete on public.user_questionnaire_progress
  from authenticated, anon, public;
revoke insert, update, delete on public.user_questionnaire_responses
  from authenticated, anon, public;
revoke insert, update, delete on public.user_questionnaire_selected_choices
  from authenticated, anon, public;
revoke insert, update, delete on public.user_questionnaire_priority_selections
  from authenticated, anon, public;

revoke all on public.user_questionnaire_progress from anon, public;
revoke all on public.user_questionnaire_responses from anon, public;
revoke all on public.user_questionnaire_selected_choices from anon, public;
revoke all on public.user_questionnaire_priority_selections from anon, public;

drop policy if exists user_questionnaire_progress_insert_own
  on public.user_questionnaire_progress;
drop policy if exists user_questionnaire_progress_update_own
  on public.user_questionnaire_progress;
drop policy if exists user_questionnaire_responses_insert_own
  on public.user_questionnaire_responses;
drop policy if exists user_questionnaire_responses_update_own
  on public.user_questionnaire_responses;
drop policy if exists user_questionnaire_responses_delete_own
  on public.user_questionnaire_responses;
...
grant select on public.user_questionnaire_progress to authenticated;
grant select on public.user_questionnaire_responses to authenticated;
grant select on public.user_questionnaire_selected_choices to authenticated;
grant select on public.user_questionnaire_priority_selections to authenticated;

do $$
begin
  if has_table_privilege('authenticated', 'public.user_questionnaire_progress', 'INSERT')
     or has_table_privilege('authenticated', 'public.user_questionnaire_progress', 'UPDATE')
     or has_table_privilege('authenticated', 'public.user_questionnaire_progress', 'DELETE')
     or has_table_privilege('authenticated', 'public.user_questionnaire_responses', 'INSERT')
     or has_table_privilege('authenticated', 'public.user_questionnaire_responses', 'UPDATE')
     or has_table_privilege('authenticated', 'public.user_questionnaire_responses', 'DELETE')
     or has_table_privilege('authenticated', 'public.user_questionnaire_selected_choices', 'INSERT')
     or has_table_privilege('authenticated', 'public.user_questionnaire_selected_choices', 'UPDATE')
     or has_table_privilege('authenticated', 'public.user_questionnaire_selected_choices', 'DELETE')
     or has_table_privilege('authenticated', 'public.user_questionnaire_priority_selections', 'INSERT')
     or has_table_privilege('authenticated', 'public.user_questionnaire_priority_selections', 'UPDATE')
     or has_table_privilege('authenticated', 'public.user_questionnaire_priority_selections', 'DELETE')
  then
    raise exception
      'authenticated still has insert/update/delete privilege on questionnaire write tables';
  end if;
end;
$$;

-- B. Idempotency table (security definer RPCs only; no authenticated access)
create table if not exists public.user_questionnaire_write_operations (
...
create index if not exists user_questionnaire_write_operations_user_created_at_idx
  on public.user_questionnaire_write_operations (user_id, created_at desc);

comment on table public.user_questionnaire_write_operations is
  'Client operation_id idempotency ledger for questionnaire write RPCs. Security definer only.';

alter table public.user_questionnaire_write_operations enable row level security;

revoke all on public.user_questionnaire_write_operations from public, anon, authenticated;

create or replace function public.forge_questionnaire_operation_result(
  p_user_id uuid,
  p_operation_id uuid
)
returns jsonb
...
begin
  ...
  select o.result
    into v_result
  from public.user_questionnaire_write_operations o
...
revoke all on function public.forge_questionnaire_operation_result(uuid, uuid)
  from public, anon, authenticated;

-- C. Completion + progress status (resume-aware; empty min=0 answers complete)
create or replace function public.forge_questionnaire_response_is_complete(
  p_response_id uuid
)
returns boolean
...
begin
  ...
  -- Tombstones / clears are never complete.
  if v_response_state = 'unanswered' then
    return false;
  end if;
  ...
  -- Answered with zero selections is complete only when min_selections = 0.
...
begin
  ...
  select (
    p.current_category_id is not null
    or p.current_question_id is not null
    or p.current_phase is not null
  )
    into v_has_resume_position
  from public.user_questionnaire_progress p
  where p.user_id = p_user_id
    and p.version_id = p_version_id;

  v_has_resume_position := coalesce(v_has_resume_position, false);
  ...
  if v_eligible_total > 0 and v_eligible_complete >= v_eligible_total then
    v_status := 'completed';
  elsif v_has_responses or v_has_resume_position then
    v_status := 'in_progress';
  else
    v_status := 'not_started';
  end if;
  ...
  set
    status = v_status,
    started_at = case
      when v_status in ('in_progress', 'completed') then coalesce(p.started_at, now())
      else p.started_at
    end,
    completed_at = case
      when v_status = 'completed' then coalesce(p.completed_at, now())
...
-- D. Save response (idempotent, empty min=0, duplicate rejection, typed JSON)
drop function if exists public.save_my_questionnaire_response(
  text, text, text[], text[], jsonb, jsonb, bigint, bigint
);
...
begin
  ...
  if p_operation_id is not null then
    v_cached := public.forge_questionnaire_operation_result(v_uid, p_operation_id);
    ...
    if p_operation_id is not null then
      ...
      begin
        insert into public.user_questionnaire_write_operations (
          operation_id, user_id, version_id, operation_kind, question_id,
          request_fingerprint, result
        ) values (
          p_operation_id, v_uid, v_version_id, 'save_response', v_question_id,
...
revoke all on function public.save_my_questionnaire_response(
  text, text, text[], text[], jsonb, jsonb, bigint, bigint, uuid
) from public, anon;
grant execute on function public.save_my_questionnaire_response(
  text, text, text[], text[], jsonb, jsonb, bigint, bigint, uuid
) to authenticated;

-- E. Clear one question (idempotent)
  ...
  if p_operation_id is not null then
    ...
    begin
      insert into public.user_questionnaire_write_operations (
        ...
        p_operation_id, v_uid, v_version_id, 'clear_question', v_question_id,
        v_fingerprint, v_result
      );
    exception
      when unique_violation then
...
revoke all on function public.clear_my_questionnaire_question(text, text, bigint, bigint, uuid)
  from public, anon;
grant execute on function public.clear_my_questionnaire_question(text, text, bigint, bigint, uuid)
  to authenticated;

-- F. Clear category / full profile (idempotent; bumps write_generation)
  ...
  if p_operation_id is not null then
    ...
    begin
      insert into public.user_questionnaire_write_operations (
        ...
        p_operation_id, v_uid, v_version_id, 'clear_category', null,
        v_fingerprint, v_result
      );
    exception
      when unique_violation then
...
revoke all on function public.clear_my_questionnaire_category(text, text, bigint, uuid)
  from public, anon;
grant execute on function public.clear_my_questionnaire_category(text, text, bigint, uuid)
  to authenticated;
  ...
  if p_operation_id is not null then
    ...
    begin
      insert into public.user_questionnaire_write_operations (
        ...
        p_operation_id, v_uid, v_version_id, 'clear_profile', null,
        v_fingerprint, v_result
      );
    exception
      when unique_violation then
...
revoke all on function public.clear_my_questionnaire_profile(text, bigint, uuid)
  from public, anon;
grant execute on function public.clear_my_questionnaire_profile(text, bigint, uuid)
  to authenticated;

-- G. Progress position: status always derived via recalculate (resume => in_progress)
create or replace function public.save_my_questionnaire_progress_position(
  p_version_key text,
  p_category_key text default null,
  p_question_key text default null,
...
begin
  ...
  -- Derive status/started_at; resume position alone yields in_progress.
  perform public.forge_recalculate_questionnaire_progress(v_uid, v_version_id);
...
-- H. Documentation: RPCs are the sole authenticated write boundary
comment on function public.save_my_questionnaire_response(
  text, text, text[], text[], jsonb, jsonb, bigint, bigint, uuid
) is
  'Authenticated write boundary: save/replace one question response. Direct table mutations are revoked.';
comment on function public.clear_my_questionnaire_question(
  text, text, bigint, bigint, uuid
) is
  'Authenticated write boundary: tombstone one question answer. Direct table mutations are revoked.';
comment on function public.clear_my_questionnaire_category(
  text, text, bigint, uuid
) is
  'Authenticated write boundary: clear one category and bump write_generation.';
comment on function public.clear_my_questionnaire_profile(
  text, bigint, uuid
) is
  'Authenticated write boundary: full Compatibility Profile clear for the active version.';
comment on function public.save_my_questionnaire_progress_position(
  text, text, text, text, bigint
) is
  'Authenticated write boundary: resume position only. Status is always server-derived.';

-- ---------------------------------------------------------------------------
-- 11. Idempotency ledger hardening: fingerprint match + post-lock recheck
-- ---------------------------------------------------------------------------

-- A. Extend ledger with optional target_key (category_key for clear_category)
alter table public.user_questionnaire_write_operations
  add column if not exists target_key text null;

comment on column public.user_questionnaire_write_operations.target_key is
  'category_key for clear_category; null otherwise.';

-- B. Replace lookup helper with fingerprint-aware resolve
drop function if exists public.forge_questionnaire_operation_result(uuid, uuid);

create or replace function public.forge_questionnaire_resolve_operation(
  p_user_id uuid,
  p_operation_id uuid,
...
declare
  ...
  v_result jsonb;
begin
  -- p_question_id and p_target_key may be null; all other args required.
  ...
  if v_kind = p_operation_kind
     and v_version_id = p_version_id
     and v_question_id is not distinct from p_question_id
     and v_target_key is not distinct from p_target_key
     and v_fingerprint = p_fingerprint then
    return v_result;
  end if;
...
revoke all on function public.forge_questionnaire_resolve_operation(
  uuid, uuid, text, uuid, uuid, text, text
) from public, anon, authenticated;

-- C. Fingerprint helpers (stable md5 of jsonb_build_object payload)
-- Inline md5(jsonb_build_object(...)::text) is used in each RPC below.

-- D. Hardened write RPCs: resolve only after progress lock; fingerprint match required
...
comment on function public.forge_questionnaire_resolve_operation(
  uuid, uuid, text, uuid, uuid, text, text
) is
  'Post-lock idempotency resolve: returns cached result on fingerprint match, idempotency_conflict on mismatch, null to proceed.';
comment on function public.save_my_questionnaire_response(
  text, text, text[], text[], jsonb, jsonb, bigint, bigint, uuid
) is
  'Authenticated write boundary: save/replace one question response. Idempotency resolved after progress lock with fingerprint match.';
comment on function public.clear_my_questionnaire_question(
  text, text, bigint, bigint, uuid
) is
  'Authenticated write boundary: tombstone one question answer. Idempotency resolved after progress lock with fingerprint match.';
comment on function public.clear_my_questionnaire_category(
  text, text, bigint, uuid
) is
  'Authenticated write boundary: clear one category and bump write_generation. target_key=category_key in ledger.';
comment on function public.clear_my_questionnaire_profile(
  text, bigint, uuid
) is
