  ...
  return (
    ...
    >
      {/*
        ...
        Desktop (lg+): balanced portrait | details columns; portrait locks to 3:4
        with object-fit cover semantics (never stretched).
      */}
      <div className="overflow-hidden rounded-[2rem] border border-[#0B2D5C]/08 bg-white/90 shadow-[0_18px_50px_rgba(11,45,92,0.08)] backdrop-blur-sm transition duration-300 hover:shadow-[0_22px_56px_rgba(11,45,92,0.12)] lg:grid lg:grid-cols-[minmax(19rem,46%)_minmax(0,1fr)] lg:items-start lg:rounded-[2.25rem] lg:shadow-[0_22px_56px_rgba(11,45,92,0.09)] lg:hover:shadow-[0_26px_64px_rgba(11,45,92,0.13)]">
        <button
          type="button"
          onClick={openProfile}
          aria-label={`${profile.firstName}'s portrait — open discovery profile`}
          className="relative block aspect-[4/5] w-full cursor-pointer overflow-hidden sm:aspect-[3/4] lg:aspect-[3/4] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#0B2D5C]"
        >
          <span className="sr-only">{profile.firstName}, placeholder portrait</span>
          {/*
            Media layer: cover + center crop.
            Real photos should use <img className="absolute inset-0 h-full w-full object-cover object-center" />.
            Placeholder uses the same cover/center behavior via background-size/position.
          */}
          <div
          ...
          <div
            ...
            aria-hidden="true"
          />
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#0B2D5C]/55 via-[#0B2D5C]/15 to-transparent px-6 pb-6 pt-28 lg:px-7 lg:pb-7 lg:pt-24">
            <h2
            ...
            <p className="mt-2 text-sm text-white/85 sm:text-base lg:text-[16px]">
              {profile.location}
            </p>
          </div>
        </button>
        ...
        <div className="min-w-0 lg:flex lg:h-full lg:flex-col">
          <button
            ...
            className="block w-full cursor-pointer space-y-6 px-5 py-6 text-left sm:px-6 sm:py-7 lg:flex lg:flex-1 lg:flex-col lg:space-y-6 lg:px-8 lg:pb-3 lg:pt-7 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#0B2D5C]"
          >
            <section aria-label="Relationship alignment">
              ...
              <p
                className="mt-2 text-xl tracking-[-0.01em] text-[#0B2D5C] sm:text-[1.35rem] lg:text-[1.45rem]"
                style={{ fontFamily: 'var(--font-discovery-display), Georgia, serif' }}
              >
            ...
            {profile.hasImportantFactors && (
              <div className="flex gap-3 rounded-2xl border-2 border-[#D62828]/80 bg-[#FBF6EE] px-4 py-3.5 lg:px-5 lg:py-3.5">
                <span
            ...
            <section aria-label="Character signals preview">
              ...
              <div className="mt-3 flex flex-wrap gap-2 lg:mt-3 lg:gap-2.5">
            ...
            <p className="text-center text-xs text-[#8A93A0] lg:hidden">
              Tap to open the full Discovery Profile
            </p>
          </button>

          <div className="space-y-3 border-t border-[#0B2D5C]/06 px-5 pb-6 pt-5 sm:px-6 sm:pb-7 lg:mt-auto lg:px-8 lg:pb-7 lg:pt-4">
            <button
              type="button"
              onClick={onInterested}
              className="inline-flex w-full items-center justify-center rounded-2xl bg-[#D62828] px-6 py-3.5 text-base font-semibold text-white shadow-[0_10px_28px_rgba(214,40,40,0.18)] transition hover:bg-[#A61F1F] active:scale-[0.99] lg:py-3.5"
            >
              Interested
            </button>
            <button
              ...
              className="inline-flex w-full items-center justify-center rounded-2xl border border-[#0B2D5C]/25 bg-white/80 px-6 py-3.5 text-base font-semibold text-[#0B2D5C] transition hover:border-[#0B2D5C]/45 hover:bg-white active:scale-[0.99] lg:py-3.5"
            >
              {openToChatLabel}
            </button>
