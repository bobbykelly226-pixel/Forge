...
describe('questionnaire catalog foundation', () => {
  it('validates the live catalog with locked Categories 1 through 10', () => {
    ...
    assert.equal(catalog.categories[0].questions.length, 3);
      ...
      categories: [
        {
          ...base.categories[0],
          status: 'draft',
          questions: base.categories[0].questions.slice(0, 2),
        },
      ],
    };
...
describe('questionnaire migration privacy and integrity', () => {
  ...
  it('seeds Category 1 title, 10 questions, and selection limits', () => {
    ...
    assert.match(migration, /commitment_partnership/);
    assert.match(migration, /family_children_parenting/);
    assert.match(migration, /faith_spirituality_worldview/);
